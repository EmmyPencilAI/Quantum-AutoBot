...
const stratIndex = content.indexOf('const changeStrategy = async (newStrategy: string) => {');
const fundIndex = content.indexOf('const fundTrading = async () => {');

const safeFundTrading = `const fundTrading = async () => {
...
if (fundIndex > -1 && stratIndex > -1) {
    content = content.slice(0, fundIndex) + safeFundTrading + content.slice(stratIndex);
